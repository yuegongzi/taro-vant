...
module.exports = function(chain) {
  ...
  chain.module.rule('global-loader').test(/node_modules\/webpack\/buildin\/global\.js/i).pre().use('global-loader').loader(npath.join(process.cwd(), 'config/webpack/globalLoader'))
  // 内部exclude = [filename => /node_modules/.test(filename) && !(/taro/.test(filename))];
  // taro编译由下面compile-node-modules来处理
  chain.module.rule('script').exclude.clear().add((filename) => /node_modules/.test(filename))
  // node_modules需要二次编译的在这里处理，taro相关的包不能加载polyfill
  chain.module.rule('compile-node-modules').test(/node_modules\/@tarojs(.+?)\.[tj]sx?$/i).use('taro-loader').loader(npath.join(process.cwd(), 'node_modules/babel-loader/lib/index.js')).options({
    presets: [
      [
        '@antmjs/babel-preset',
        {
