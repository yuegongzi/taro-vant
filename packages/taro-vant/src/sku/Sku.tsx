...
const CART: SkuAction = { text: '立即购买', visible: true }
const BUY: SkuAction = { text: '加入购物车', visible: true }
...
const Sku: FC<SkuProps> = (props) => {
  ...
  const selectedCheckFn = () => {
    if (selectedSkuComb) {
      ...
      const prop = selectedPropValues.
        map((item: SpuSpec) => item.name).
        join(' ')
      return `已选 ${spec} ${prop}`
    }
    const unselectedSku = tree.
      filter(
        (item: SpuSpec) =>
          selectedSku[item.id] === '' || selectedSku[item.id] == null,
      ).
      map((item: SpuSpec) => item.name)
